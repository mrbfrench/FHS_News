...
function displaySubPeriodOptions(period) {
    if (period == undefined) {replaceAllChildren(subBox, []);return;}
    let buttons = [];
    let i = 0;
...
function arrowAdvancePeriod(direction) {
    let skip = false;
    if (direction > 0) {
        if (index_obj.main_index >= schedule.periods.length - 1) {return;}
        if (index_obj.main_index == -1) {index_obj.main_index = 0;skip = true;}
    } else if (direction < 0) {
        if (index_obj.main_index <= 0) {return;}
        if (index_obj.main_index == schedule.periods.length) {index_obj.main_index = schedule.periods.length - 1; skip = true;}
    }
    let period = schedule.periods[index_obj.main_index];
    periodBox.children[2].children[index_obj.main_index].classList = "gallery-dot";
    if (!skip) {
